...
class AscFile:
    ...
    def get_properties(self):
        ...
        with open(self.path, "r") as f:
            ...
            self.header = [
                x[:-1] for x in raw_header
            ]  # removing '\n' for the end of each line
            values = [
                float(list(item.split(" "))[-1]) for item in self.header
    ...
    def get_data(self):
        ...
        with open(self.path) as f:
            ...
            flattened_data = [float(x) for line in raw_data for x in line.split() if x]
    ...
    def generate_random_grid(nrows=5, ncols=5, no_data_value=-9999):
        ...
        with open("random_grid.asc", "w") as f:
...
class MyASCII:
    ...
    def write_asc(self):
        ...
        data = np.flip(
            np.squeeze(np.nan_to_num(da.to_numpy(), nan=self.nodata_value)), 0
        )
